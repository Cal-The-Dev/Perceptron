...
class Perceptron():
    ...
    def __init__ (self, DataLength,regularisation):
        ...
        for iteration in range(MaximumIterations):
            for row in range(DataLength):
                ...
                if (Labels[row]*self.Activation) <= 0:
                    ...
                    #self.L2term = self.Regularisation / (Labels[row] * 2) *(np.sum(np.square(Weights[0])) + np.sum(np.square(Weights[1])) + np.sum(np.square(Weights[2])) + np.sum(np.square(Weights[3])) )
                    
                    for weight in range(4):
                        self.Weights[weight] =  ( (1 - 2*self.Regularisation)*self.Weights[weight] ) + (Labels[row] * TrainingData[row,weight])
                        
                    
                    #for weight in range(4):
                        #self.Weights[weight] = (self.Weights[weight]/self.normalisation)
...
testingData = np.zeros((len(testingFileShuffle), 4))
testingOutputs = np.zeros(len(testingFileShuffle))
...
Perceptron1 = Perceptron(4,regularisation)
Perceptron2 = Perceptron(4,regularisation)
Perceptron3 = Perceptron(4,regularisation)
